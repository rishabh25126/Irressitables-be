...
const getManageableOne = asyncHandler(async (req, res) => {
  ...
  const owners = await OwnerAccess.find({ businessId: business._id }).populate('ownerId', 'name email role');
...
const create = asyncHandler(async (req, res) => {
  ...
  const owners = await User.find({ _id: { $in: ownerIds }, role: 'owner' }).select('_id');
...
const update = asyncHandler(async (req, res) => {
  ...
  const business = await Business.findByIdAndUpdate(req.params.id, { $set: payload }, { new: true, runValidators: true });
...
const togglePublish = asyncHandler(async (req, res) => {
  ...
  return success(res, { isPublished: business.isPublished }, `Business ${state}.`);
});

module.exports = { getAll, getManageableBusinesses, getOne, getManageableOne, create, update, remove, togglePublish };

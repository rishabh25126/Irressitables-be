...
const connectDB = async (retriesLeft = defaultRetryCount()) => {
  ...
  if (connectMutex) {
    ...
    throw new Error(`MongoDB not connected after wait (readyState=${mongoose.connection.readyState})`);
  }
  ...
  connectMutex = (async () => {
    ...
    while (attempts >= 0) {
      try {
        ...
        if (mongoose.connection.readyState !== 1) {
          throw new Error(`Mongo readyState ${mongoose.connection.readyState} after connect()`);
        }
